...
Before starting, ensure you have completed the [Installation Guide](./installation.md) with both the broker and at least one agent running. You will need the admin PAK (Prefixed API Key) from the broker setup and `kubectl` configured to access your target cluster. The examples below assume the broker is accessible at `http://localhost:3000` via port-forward.
...
- Check the [Work Orders](../reference/work-orders.md) reference for container builds and other transient operations
