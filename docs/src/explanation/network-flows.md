...
### What Else That Port Carries

Port 3000 is not only the JSON API, and the difference matters when choosing between the exposure methods above.

**The Operator Console is served from the same port.** The published broker image bundles a read-only web console and serves it at the broker's root URL, with the API routes taking precedence over it. The console needs no credential of its own: the page the broker hands the browser carries an ephemeral read-only administrative token, minted fresh in memory each time the broker process starts and never written to the database. That token is, by design, visible to anyone who can fetch the page. **Network reach is the console's entire authentication boundary.** The credential cannot change anything — reads are allowed, mutations are refused — but it can see the whole deployment: every tenant's stacks, the fleet rollup, telemetry, and work orders. Choosing LoadBalancer or an unrestricted Ingress therefore grants read-only visibility of the entire system to whoever can reach the address, which is a decision to make deliberately rather than inherit. Restricting access at the ingress, or leaving the service ClusterIP and reaching the console through a port-forward, keeps that boundary where most operators would want it.

Two consequences follow for exposure planning. Because each broker replica mints its own token, a browser that loads the console from one replica and then talks to another is rejected — a multi-replica console needs **session affinity** at the load balancer or ingress. And because the served page embeds a process-lifetime credential, it is sent with caching disabled; any proxy in front of the broker must not cache the root document.

**Consumer WebSockets share the port too.** Beyond the internal broker↔agent channel described below, the broker serves two consumer-facing WebSocket upgrades on 3000: a per-stack live event and log tail, and a fleet-wide live update stream. Both authenticate, but browsers cannot set an `Authorization` header on a WebSocket, so they pass their credential in the `Sec-WebSocket-Protocol` header instead. Anything terminating TLS in front of the broker must forward that header intact along with the upgrade, and tolerate long-lived idle connections.

**`/metrics` is unauthenticated.** The Prometheus scrape endpoint sits outside the authenticated API and answers any request that reaches it. It exposes operational shape — request rates and latencies by path, connected-agent counts, eviction-worker activity — rather than deployment content, but it is still not something to publish to the internet. Keep it reachable from the monitoring system and no further; the [Network Configuration how-to](../how-to/network-configuration.md) covers restricting it with a NetworkPolicy.
