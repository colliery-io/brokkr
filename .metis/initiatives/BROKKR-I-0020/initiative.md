...
- [x] A2 chaos test green in CI (BROKKR-T-0171, 2026-05-24) — REST-heartbeat-fallback proven. Work-order-completion lifecycle was deferred here and **subsequently delivered in BROKKR-T-0183** (see below)
...
- [x] B2 Grafana dashboard + eviction-worker-dead alert (BROKKR-T-0175, 2026-05-26) — `docs/grafana/brokkr-ws-channel-dashboard.json` (5 panels) + `brokkr-ws-channel.rules.yml` (`BrokkrWsEvictionWorkerStalled`, fires when eviction stops >10m). Queries verified against live `/metrics`; docs note added. Visual Grafana render deferred (no Grafana/Prometheus in local stack)
- [x] B3 PAK revocation closes WS socket (BROKKR-T-0176, 2026-05-26) — `ConnectionRegistry::close_for_agent` wired into `rotate_agent_pak` + `delete_agent` after commit; dropping the handle's lane senders is the close signal (no new cancel token). Unit + 2 integration tests green (registry clears + socket closes <1s); docs note added. Reconnect-after-revoke backs off via existing jittered backoff (no tight-loop)
...
- [x] C1 browser live-tail (BROKKR-T-0178, 2026-05-26) — approach (a) Sec-WebSocket-Protocol PAK auth (ADR-0008 amendment); broker `ws_subprotocol_auth` + marker echo, ui-slim "Go Live" tail. 4 live-subscription tests green incl. subprotocol auth + bad-PAK reject; header path unchanged
