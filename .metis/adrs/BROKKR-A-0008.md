...
### 2026-05-26 — Browser WS auth via `Sec-WebSocket-Protocol` (BROKKR-T-0178 / I-0020 C1)

WS-12 deferred the browser-side live tail because the browser `WebSocket` API can't set request headers, so the PAK can't ride on `Authorization`. **Decision (approved): accept the PAK via the `Sec-WebSocket-Protocol` header** on the live-tail endpoint (`/api/v1/stacks/{id}/live`). The browser offers two subprotocols — a non-secret marker `brokkr.v1` and `brokkr.pak.<PAK>` — and the broker lifts the PAK out of the latter into an `Authorization: Bearer` header before the normal auth middleware runs, echoing back only the marker (never the secret) in the handshake response.

**Rejected alternative:** a server-side SSE proxy in `ui-slim` that opens the upstream WS with the PAK and re-streams as SSE. It keeps the broker's auth surface unchanged but doubles the proxy surface area for what is a transport-level limitation, and splits "who holds auth" across two services.

**Rationale:** the subprotocol approach is the industry-standard browser workaround (the Kubernetes API server does exactly this) and keeps the broker the single source of truth for auth. The cost is a small amount of non-obvious auth code on the broker, isolated to the subscribe route. The change is **purely additive** — the existing header-based path (agents, SDKs, the Node `ws` contract test) is untouched; the subprotocol PAK is only consulted when no `Authorization` header is present. Implemented in `ws/subscribe.rs` (`ws_subprotocol_auth` + marker echo) with `ui-slim`'s live-tail view; covered by `live_subscription_authenticates_via_subprotocol`.