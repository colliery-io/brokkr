...
async fn test_list_agent_events() {
    ...
    let test_deployment_object = fixture.create_test_deployment_object(
        test_stack.id,
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/v1/agents/{}/events?status=SUCCESS",
                    test_agent.id
                ))
                .body(Body::empty())
                .unwrap(),
...
async fn test_list_stack_events() {
    ...
    let test_agent1 =
        fixture.create_test_agent("Test Agent 1".to_string(), "Test Cluster".to_string());
    let test_agent2 =
        fixture.create_test_agent("Test Agent 2".to_string(), "Test Cluster".to_string());
    let test_stack = fixture.create_test_stack("Test Stack".to_string(), None);
    let test_deployment_object = fixture.create_test_deployment_object(
        test_stack.id,
        "yaml_content: test".to_string(),
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!("/api/v1/stacks/{}/events", test_stack.id))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn test_get_agent_event() {
    ...
    assert_eq!(
        retrieved_event.deployment_object_id,
