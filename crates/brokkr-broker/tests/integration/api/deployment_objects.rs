...
async fn test_list_deployment_objects_by_agent() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/v1/deployment-objects?agent_id={}",
                    test_agent.id
                ))
                .body(Body::empty())
                .unwrap(),
...
async fn test_list_deployment_objects_by_stack() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/v1/deployment-objects?stack_id={}",
                    test_stack.id
                ))
                .body(Body::empty())
                .unwrap(),
...
async fn test_list_deployment_objects_bad_request() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!(
                    "/api/v1/deployment-objects?agent_id={}&stack_id={}",
...
async fn test_get_deployment_object() {
    ...
    let test_stack = fixture.create_test_stack("Test Stack".to_string(), None);
    let test_object = fixture.create_test_deployment_object(
        test_stack.id,
        "yaml_content: test_object".to_string(),
        false,
    );
...
async fn test_delete_deployment_object() {
    ...
    let test_object = fixture.create_test_deployment_object(
        test_stack.id,
        "yaml_content: to_be_deleted".to_string(),
        false,
    );
    ...
    let deleted_object = fixture
        .dal
        .deployment_objects()
        .get_including_deleted(test_object.id)
        .unwrap()
        .unwrap();
