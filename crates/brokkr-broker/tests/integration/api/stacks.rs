...
async fn test_list_stacks() {
    ...
    let stack1 = fixture.create_test_stack(
        "Test Stack 1".to_string(),
        Some("Description 1".to_string()),
    );
    let stack2 = fixture.create_test_stack(
        "Test Stack 2".to_string(),
    ...
    assert!(
        stacks.len() >= 3,
        "Expected at least 3 stacks, got {}",
        stacks.len()
    );
...
async fn test_get_stack() {
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri(format!("/stacks/{}", test_stack.id))
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn test_update_stack() {
    ...
    // Create a test stack
    let original_stack = fixture.create_test_stack(
        "Original Stack Name".to_string(),
    ...
    assert_eq!(
        updated_stack.description,
        Some("Updated description".to_string())
    );

    // Verify the update in the database
    let db_stack = fixture
        .dal
        .stacks()
        .get(original_stack.id)
        .unwrap()
        .unwrap();
    assert_eq!(db_stack.name, "Updated Stack Name");
    assert_eq!(
        db_stack.description,
        Some("Updated description".to_string())
    );
}
...
async fn test_list_deployment_objects() {
    ...
    // Create a test stack
    let test_stack =
        fixture.create_test_stack("Test Stack for Deployment Objects".to_string(), None);
    ...
    assert_eq!(
        ...
        3,
        "Expected 3 deployment objects, got {}",
        deployment_objects.len()
    );
    ...
    // Verify object1
    let found_object1 =
        find_object(object1.id).expect("Deployment object 1 not found in the response");
    assert_eq!(found_object1.yaml_content, "yaml_content: object1");
    assert!(!found_object1.is_deletion_marker);

    // Verify object2
    let found_object2 =
        find_object(object2.id).expect("Deployment object 2 not found in the response");
    ...
    let found_object3 = find_object(object3.id)
        .expect("Deployment object 3 (deletion marker) not found in the response");
    ...
    for obj in &deployment_objects {
        assert_eq!(
            obj.stack_id, test_stack.id,
            "Deployment object {} does not belong to the correct stack",
            obj.id
        );
    }

    // Verify that the deployment objects are returned in descending order of sequence_id
    let sequence_ids: Vec<i64> = deployment_objects
        .iter()
        .map(|obj| obj.sequence_id)
        .collect();
    assert!(
        sequence_ids.windows(2).all(|w| w[0] >= w[1]),
...
async fn test_create_deployment_object() {
    ...
    let new_deployment_object =
        NewDeploymentObject::new(test_stack.id, "test yaml content".to_string(), false)
...
async fn test_list_annotations() {
    ...
    assert!(
        annotations
            .iter()
            .any(|a| a.id == annotation1.id && a.key == "key1" && a.value == "value1"),
        "Annotation 1 not found in the response"
    );
    assert!(
        annotations
            .iter()
            .any(|a| a.id == annotation2.id && a.key == "key2" && a.value == "value2"),
    ...
    for annotation in annotations {
        assert_eq!(
            annotation.stack_id, test_stack.id,
            "Annotation {} does not belong to the correct stack",
            annotation.id
        );
    }
}
