...
fn test_get_undeployed_objects_for_agent() {
    ...
    let object1 = fixture.create_test_deployment_object(stack1.id, "yaml_content: object1".to_string(), false);
    let object2 = fixture.create_test_deployment_object(stack1.id, "yaml_content: object2".to_string(), false);
    let object3 = fixture.create_test_deployment_object(stack2.id, "yaml_content: object3".to_string(), false);
    ...
    let undeployed_objects = fixture.dal.deployment_objects().get_undeployed_objects_for_agent(agent.id)
    ...
    assert!(undeployed_ids.contains(&object2.id), "object2 should be undeployed");
    assert!(undeployed_ids.contains(&object3.id), "object3 should be undeployed");
    
    // Check that object1 is not in the undeployed objects list
    assert!(!undeployed_ids.contains(&object1.id), "object1 should not be undeployed");
    
    // Verify that the objects are sorted by sequence_id in descending order
    assert!(undeployed_objects[0].sequence_id > undeployed_objects[1].sequence_id, 
...
fn test_get_undeployed_objects_for_agent_with_no_targets() {
    ...
    let agent = fixture.create_test_agent("Agent No Targets".to_string(), "Test Cluster".to_string());
    ...
    // Verify the results
    assert!(undeployed_objects.is_empty(), "Expected no undeployed objects for agent without targets");
}
...
fn test_get_undeployed_objects_for_agent_with_all_deployed() {
    ...
    // Create an agent
    let agent = fixture.create_test_agent("Agent All Deployed".to_string(), "Test Cluster".to_string());
    ...
    let object1 = fixture.create_test_deployment_object(stack.id, "yaml_content: object1".to_string(), false);
    let object2 = fixture.create_test_deployment_object(stack.id, "yaml_content: object2".to_string(), false);
    ...
    // Verify the results
    assert!(undeployed_objects.is_empty(), "Expected no undeployed objects when all are deployed");
}
...
fn test_get_undeployed_objects_for_agent_with_deletion_markers() {
    ...
    // Create an agent
    let agent = fixture.create_test_agent("Agent Deletion Markers".to_string(), "Test Cluster".to_string());
    ...
    let object1 = fixture.create_test_deployment_object(stack.id, "yaml_content: object1".to_string(), false);
    let object2 = fixture.create_test_deployment_object(stack.id, "yaml_content: object2".to_string(), false);
    let deletion_marker = fixture.create_test_deployment_object(stack.id, "yaml_content: deletion_marker".to_string(), true);
    ...
    // Verify the results
    assert_eq!(undeployed_objects.len(), 2, "Expected 2 undeployed objects (including deletion marker)");
    
    let undeployed_ids: Vec<uuid::Uuid> = undeployed_objects.iter().map(|obj| obj.id).collect();
    assert!(undeployed_ids.contains(&object2.id), "object2 should be undeployed");
    assert!(undeployed_ids.contains(&deletion_marker.id), "deletion marker should be included");
    assert!(!undeployed_ids.contains(&object1.id), "object1 should not be undeployed");
    
    // Verify that the deletion marker is included and has the correct flag
    let deletion_marker_result = undeployed_objects.iter().find(|obj| obj.id == deletion_marker.id).unwrap();
    assert!(deletion_marker_result.is_deletion_marker, "Deletion marker should have is_deletion_marker set to true");
}
...
fn test_search_deployment_objects_by_checksum() {
    ...
    let object1 = fixture.create_test_deployment_object(stack.id, "yaml_content: object1".to_string(), false);
    let object2 = fixture.create_test_deployment_object(stack.id, "yaml_content: object2".to_string(), false);
    let object3 = fixture.create_test_deployment_object(stack.id, "yaml_content: object1".to_string(), false);

    // Search for objects with the same checksum as object1
    let search_results = fixture.dal.deployment_objects().search(&object1.yaml_checksum)
        .expect("Failed to search deployment objects");

    // Verify the results
    assert_eq!(search_results.len(), 2, "Expected 2 objects with the same checksum");
    assert!(search_results.iter().any(|obj| obj.id == object1.id), "object1 should be in the search results");
    assert!(search_results.iter().any(|obj| obj.id == object3.id), "object3 should be in the search results");
    assert!(!search_results.iter().any(|obj| obj.id == object2.id), "object2 should not be in the search results");

    // Verify that the objects are sorted by sequence_id in descending order
    assert!(search_results[0].sequence_id > search_results[1].sequence_id, 
            "Search results should be sorted by sequence_id in descending order");

    // Search for a non-existent checksum
    let non_existent_search = fixture.dal.deployment_objects().search("non_existent_checksum")
        .expect("Failed to search for non-existent checksum");
    assert!(non_existent_search.is_empty(), "Search for non-existent checksum should return empty results");
}
...
fn test_get_applicable_deployment_objects() {
    ...
    let applicable_objects = fixture.dal.deployment_objects().get_applicable_deployment_objects(agent.id)
    ...
    assert!(object_ids.contains(&object1.id), "object1 should be applicable");
    assert!(object_ids.contains(&object2.id), "object2 should be applicable");
    assert!(object_ids.contains(&object3.id), "object3 should be applicable");
    
    // Verify that the objects are sorted by sequence_id in descending order
    assert!(applicable_objects[0].sequence_id >= applicable_objects[1].sequence_id, 
            "Applicable objects should be sorted by sequence_id in descending order");
    assert!(applicable_objects[1].sequence_id >= applicable_objects[2].sequence_id, 
            "Applicable objects should be sorted by sequence_id in descending order");
}