...
async fn get_deployment_object(
    ...
    let deployment_object = dal.deployment_objects().get(id)
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?
...
async fn delete_deployment_object(
    ...
    dal.deployment_objects().soft_delete(id)
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(()))
}
...
pub fn configure_routes() -> Router<DAL> {
    Router::new()
        .route("/deployment-objects/:id", get(get_deployment_object).delete(delete_deployment_object))
}