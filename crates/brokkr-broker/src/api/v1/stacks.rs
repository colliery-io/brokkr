...
async fn list_deployment_objects(Path(id): Path<String>) -> Json<Value> {
    Json(
        serde_json::json!({"message": format!("List deployment objects for stack with ID: {}", id)}),
    )
}

async fn create_deployment_object(Path(id): Path<String>) -> Json<Value> {
    Json(
        serde_json::json!({"message": format!("Create deployment object for stack with ID: {}", id)}),
    )
}
...
async fn remove_label(Path((id, label)): Path<(String, String)>) -> Json<Value> {
    Json(
        serde_json::json!({"message": format!("Remove label '{}' from stack with ID: {}", label, id)}),
    )
}
...
async fn remove_annotation(Path((id, key)): Path<(String, String)>) -> Json<Value> {
    Json(
        serde_json::json!({"message": format!("Remove annotation '{}' from stack with ID: {}", key, id)}),
    )
}
