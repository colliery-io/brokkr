...
pub fn routes() -> Router {
    Router::new()
        ...
        .route("/agents/:id/annotations", get(list_annotations).post(add_annotation))
...
async fn remove_label(Path((id, label)): Path<(String, String)>) -> Json<Value> {
    Json(serde_json::json!({"message": format!("Remove label '{}' from agent with ID: {}", label, id)}))
}
...
async fn remove_annotation(Path((id, key)): Path<(String, String)>) -> Json<Value> {
    Json(serde_json::json!({"message": format!("Remove annotation '{}' from agent with ID: {}", key, id)}))
}
...
async fn remove_target(Path((id, stack_id)): Path<(String, String)>) -> Json<Value> {
    Json(serde_json::json!({"message": format!("Remove target '{}' from agent with ID: {}", stack_id, id)}))
}
...
async fn get_applicable_deployment_objects(Path(id): Path<String>) -> Json<Value> {
    Json(serde_json::json!({"message": format!("Get applicable deployment objects for agent with ID: {}", id)}))
}
