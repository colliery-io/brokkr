...
mod tests {
    ...
    fn test_new_stack_label_success() {
        ...
        assert!(result.is_ok(), "NewStackLabel creation should succeed with valid inputs");
        let new_label = result.unwrap();
        assert_eq!(new_label.stack_id, stack_id, "stack_id should match the input value");
    ...
    fn test_new_stack_label_invalid_stack_id() {
        ...
        assert!(result.is_err(), "NewStackLabel creation should fail with nil stack ID");
    ...
    fn test_new_stack_label_empty_label() {
        ...
        assert!(result.is_err(), "NewStackLabel creation should fail with empty label");
    ...
    fn test_new_stack_label_whitespace_label() {
        let result = NewStackLabel::new(Uuid::new_v4(), "   ".to_string());
        assert!(result.is_err(), "NewStackLabel creation should fail with whitespace-only label");
        assert_eq!(result.unwrap_err(), "Label cannot be empty", "Error message should indicate empty label");
    }
    ...
    fn test_new_stack_label_too_long() {
        ...
        assert!(result.is_err(), "NewStackLabel creation should fail with label exceeding 64 characters");
    ...
    fn test_new_stack_label_max_length() {
        ...
        assert!(result.is_ok(), "NewStackLabel creation should succeed with 64-character label");
    }
}