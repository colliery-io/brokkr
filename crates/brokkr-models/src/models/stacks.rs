...
mod tests {
    ...
    fn test_new_stack_success() {
        ...
        assert!(result.is_ok(), "NewStack creation should succeed with valid inputs");
    ...
    fn test_new_stack_empty_name() {
        ...
        assert_eq!(result.unwrap_err(), "Stack name cannot be empty", "Error message should indicate empty name");
    }

    #[test]
    fn test_new_stack_empty_description() {
        let result = NewStack::new("Valid Name".to_string(), Some("".to_string()));
        assert!(result.is_err(), "NewStack creation should fail with empty description");
        assert_eq!(
            result.unwrap_err(),
