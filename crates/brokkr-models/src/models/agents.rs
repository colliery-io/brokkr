...
mod tests {
    ...
    fn test_new_agent_success() {
        ...
        let result = NewAgent::new(name.clone(), cluster_name.clone());

        assert!(
            result.is_ok(),
            "NewAgent creation should succeed with valid inputs"
        );
        ...
        assert_eq!(
            ...
            "cluster_name should match the input value"
        );
    }

    #[test]
    fn test_new_agent_empty_name() {
        let result = NewAgent::new("".to_string(), "Test Cluster".to_string());
        assert!(
            result.is_err(),
    ...
    fn test_new_agent_empty_cluster_name() {
        let result = NewAgent::new("Test Agent".to_string(), "".to_string());
        assert!(
            result.is_err(),
