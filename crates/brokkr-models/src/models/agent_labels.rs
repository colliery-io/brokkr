...
mod tests {
    ...
    fn test_new_agent_label_success() {
        ...
        assert!(result.is_ok(), "NewAgentLabel creation should succeed with valid inputs");
        let new_label = result.unwrap();
        assert_eq!(new_label.agent_id, agent_id, "agent_id should match the input value");
    ...
    fn test_new_agent_label_invalid_agent_id() {
        ...
        assert!(result.is_err(), "NewAgentLabel creation should fail with nil agent ID");
    ...
    fn test_new_agent_label_empty_label() {
        ...
        assert!(result.is_err(), "NewAgentLabel creation should fail with empty label");
    ...
    fn test_new_agent_label_too_long() {
        ...
        assert!(result.is_err(), "NewAgentLabel creation should fail with label exceeding 64 characters");
        assert_eq!(result.unwrap_err(), "Label cannot exceed 64 characters", "Error message should indicate label is too long");
    }
    ...
    fn test_new_agent_label_with_whitespace() {
        ...
        assert!(result.is_err(), "NewAgentLabel creation should fail with label containing whitespace");
        assert_eq!(result.unwrap_err(), "Label cannot contain whitespace", "Error message should indicate label contains whitespace");
    }
    ...
    fn test_new_agent_label_max_length() {
        ...
        assert!(result.is_ok(), "NewAgentLabel creation should succeed with 64-character label");
    }
}